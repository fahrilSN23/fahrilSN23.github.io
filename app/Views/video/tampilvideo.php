...
<div class="container-fluid">
    <div class="container">
        <nav class="breadcrumb bg-transparent m-0 p-0">
            ...
            <a class="breadcrumb-item active" href="#"><?=$listplay->jdl_playlist?></a>
        </nav>
    </div>
</div>
<!-- Breadcrumb End -->
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="d-flex align-items-center justify-content-between bg-light py-2 px-4 mb-3">
                            <h3 class="m-0">Video <?=$listplay->jdl_playlist?></h3>
                        </div>
                    </div>
                    <?php foreach ($video as $vid) {
                    $keterangan =(strip_tags($vid->keterangan)); 
                    $isi = substr($keterangan,0,220); 
                    $isi = substr($keterangan,0,strrpos($isi," "));    
                    ?>
                    <div class="col-lg-4">
                        <div class="position-relative mb-3">
                            <img class="img-fluid w-100" src="<?=base_url()?>public/template/assets/img/video/<?=$vid->gbr_video?>" style="object-fit: cover;">
                            <div class="overlay position-relative bg-light">
                                <div class="mb-2" style="font-size: 14px;">
                                    <a href="<?=site_url('listvideo/detail/'.$listplay->playlist_seo.'/'.$vid->video_seo)?>"><?=$vid->email?></a>
                                    <span class="px-1">/</span>
                                    <span><?= format_tgl($vid->tanggal) ?></span>
                                </div>
                                <a class="h4" href="<?=site_url('listvideo/detail/'.$listplay->playlist_seo.'/'.$vid->video_seo)?>"><?=$vid->jdl_video?></a>
                                <p class="m-0"><?=$isi?>...</p>
                            </div>
                        </div>
                    </div>
